  ...
  if (userData.length != 0) {
    if (!PWDTool.isPendingPasswordReset(userData.userName)) {
      ...
      databaseConnection.query(
        `INSERT INTO securityprojectdb.forgetPassword(userName,pincodeHash,pincodeSalt) VALUES ('${userData.userName}','${pincodeHash}','${pincodeSalt}')`
      );
      res
        .status(200)
    ...
    if (req.body.password && req.body.pincode) {
      ...
      if (PWDTool.isPendingPasswordReset(username)) {
        var pendingData = databaseConnection.query(
          `SELECT pincodeHash,pincodeSalt FROM securityprojectdb.forgetPassword WHERE userName = '${username}'`
        )[0];
        if (
          ...
          if (!PWDTool.isComplexed(req.body.password)) {
            ...
            PWDTool.changePassword(username, req.body.password);
            databaseConnection.query(
              `DELETE FROM securityprojectdb.forgetPassword WHERE userName = '${username}'`
            );
            res.status(200).send(`Password changed successfully!`);
          }
        } else {
